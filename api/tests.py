...
class ProfissionalTests(APITestCase):
    ...
    def setUp(self):
        ...
        self.profissional_data = {'nome_social': 'Dr. James', 'profissao': 'oncologista', 'endereco': 'Rua do james, 123', 'contato': '987654321'}
    ...
    def test_update_profissional(self):
        url = reverse('profissional-detail', kwargs={'pk':self.profissional.pk})
        updated_data = {'nome_social': 'Dr. House', 'profissao': 'nefrologista', 'endereco': 'Rua do house, 123', 'contato': '111111111'}
...
class ConsultaTests(APITestCase):
    #Nesta classe vamos testar apenas a parte unica dela, já que o resto dos metodos ja funciona
    def setUp(self):
        self.user = User.objects.create_user(username='testuser2', password='testpassword123')
        self.client.force_authenticate(user=self.user)
        self.profissional = Profissional.objects.create(nome_social='Dra. Lisa Cuddy', profissao='Administradora', endereco='Hospital Plainsboro', contato='555')
        self.consulta = Consulta.objects.create(profissional=self.profissional, data_consulta=timezone.now())

    def test_get_consultas_por_profissional(self):
        outro_profissional = Profissional.objects.create(nome_social='Dra. Lisa Lisa', profissao='Recepcionista', endereco='Hospital Plainsboro', contato='777')
        Consulta.objects.create(profissional=outro_profissional, data_consulta= timezone.now())
    ...
    def test_create_consulta_invalid_profissional(self):
        ...
        fake_uuid = uuid.uuid4()
        data = {'profissional': str(fake_uuid), 'data_consulta': timezone.now().isoformat()}
        response = self.client.post(url, data, format='json')
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)